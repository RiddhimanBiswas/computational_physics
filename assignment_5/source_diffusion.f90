program diffusion_source
    ...
    print *, "Enter the source term to add after every 100 steps"
    read *, source
    ...
    write(filename, '(A, F0.3, A, F0.3, A, F0.3, A)') &
    ...
    do iter = 1, niter
        ...
        do i = 1, nx
            do j = 1, ny
                a(i,j) = a_old(i,j) + dt*(d_a*laplace_a(i,j))
                b(i,j) = b_old(i,j) + dt*(d_b*laplace_b(i,j))
            end do
        end do
        
        ! adding a small source term
        ! a(int(nx/2),int(ny/2)) = a(int(nx/2),int(ny/2)) + 0.1

        if (iter == next_save) then
            time = iter * dt
            write(10,*) time, a, b
            ! uncomment next line for periodic adding of source
            a(int(nx/2),int(ny/2)) = a(int(nx/2),int(ny/2)) + source
